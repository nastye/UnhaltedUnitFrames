...
local function AddFakeAura(frame, num, isDebuff, Unit)
    if not frame then return end

    local config = isDebuff and UUF.DB.global[Unit].Debuffs or UUF.DB.global[Unit].Buffs
    if not config then return end

    local growthX = config.GrowthX or "RIGHT"
    local growthY = config.GrowthY or "DOWN"
    local anchorFrom = config.AnchorFrom or "TOPLEFT"
    local anchorTo = config.AnchorTo or "TOPLEFT"
    ...
    local _, parentFrame, _, _, _ = frame:GetPoint()
    ...
    for i = 1, num do
        ...
        if not button then
            ...
            local countConfig = config.Count or {}
            button.Count:SetPoint(
                countConfig.AnchorFrom or "BOTTOMRIGHT",
                button,
                countConfig.AnchorTo or "BOTTOMRIGHT",
                countConfig.XOffset or 0,
                countConfig.YOffset or 0
            )
            button.Count:SetFont(UUF.DB.global.General.Font or "Fonts\\FRIZQT__.TTF", countConfig.FontSize or 10, "OUTLINE")
        ...
        if i == 1 then
            button:SetPoint(anchorFrom, parentFrame, anchorTo, xOffset, yOffset)
        else
            local prev = frame[i - 1]
            if growthX == "LEFT" then
                button:SetPoint("RIGHT", prev, "LEFT", -spacing, 0)
            elseif growthX == "RIGHT" then
                button:SetPoint("LEFT", prev, "RIGHT", spacing, 0)
            elseif growthY == "UP" then
                button:SetPoint("BOTTOM", prev, "TOP", 0, spacing)
            elseif growthY == "DOWN" then
                button:SetPoint("TOP", prev, "BOTTOM", 0, -spacing)
            end
        end

        button.Icon:SetTexture(isDebuff and "Interface\\Icons\\Spell_Nature_ResistNature" or "Interface\\Icons\\Spell_Nature_Regeneration")
        ...
        PostCreateButton(nil, button, Unit, auraType)
        PostUpdateButton(nil, button, Unit, auraType)

        button:Show()
    end
end

function UUF:TestMode()
    if not UUF.DB.global.TestMode then
        UUF:CreateReloadPrompt()
    else
        for _, Unit in pairs(Frames) do
            local UnitDB = UUF.DB.global[Unit]
            if UnitDB then
                local General = UUF.DB.global.General
                local Frame = UnitDB.Frame
                local Portrait = UnitDB.Portrait
                local Health = UnitDB.Health
                local PowerBar = UnitDB.PowerBar
                local Absorbs = UnitDB.Health.Absorbs
                local Buffs = UnitDB.Buffs
                local Debuffs = UnitDB.Debuffs
                local TargetMarker = UnitDB.TargetMarker
                local LeftText = UnitDB.Texts.Left
                local RightText = UnitDB.Texts.Right
                ...
                local TestFrame = _G["UUF_" .. Unit]
                if TestFrame then
                    UnregisterUnitWatch(TestFrame)
                    TestFrame:UnregisterAllEvents()
                    TestFrame.unit = nil
                    TestFrame:Show()
                    TestFrame:SetAlpha(1)
                    TestFrame:SetScript("OnEnter", nil)
                    TestFrame:SetScript("OnLeave", nil)
                    TestFrame:EnableMouse(false)
                    GameTooltip:Hide()

                    if Portrait.Enabled and TestFrame.unitPortrait then
                        TestFrame.unitPortrait:SetTexture("Interface\\ICONS\\INV_Misc_QuestionMark")
                        TestFrame.unitPortrait:Show()
                    end

                    if PowerBar.Enabled and TestFrame.unitPowerBar then
                        TestFrame.unitPowerBarBackdrop:ClearAllPoints()
                        TestFrame.unitPowerBarBackdrop:SetSize(Frame.Width, PowerBar.Height)
                        TestFrame.unitPowerBarBackdrop:SetPoint("BOTTOMLEFT", TestFrame, "BOTTOMLEFT", 0, 0)
                        TestFrame.unitPowerBarBackdrop:SetBackdrop(BackdropTemplate)
                        TestFrame.unitPowerBarBackdrop:SetBackdropColor(unpack(PowerBar.BackgroundColour))
                        TestFrame.unitPowerBarBackdrop:SetBackdropBorderColor(unpack(General.BorderColour))
                        TestFrame.unitPowerBarBackdrop:SetFrameLevel(4)
                        TestFrame.unitPowerBar:ClearAllPoints()
                        TestFrame.unitPowerBar:SetPoint("BOTTOMLEFT", TestFrame, "BOTTOMLEFT", 1, 1)
                        TestFrame.unitPowerBar:SetSize(Frame.Width - 2, PowerBar.Height - 2)
                        TestFrame.unitPowerBar:SetStatusBarTexture(General.ForegroundTexture)
                        TestFrame.unitPowerBar:SetStatusBarColor(unpack(General.CustomColours.Power[0]))
                        TestFrame.unitPowerBar:SetMinMaxValues(0, 100)
                        TestFrame.unitPowerBar:SetValue(math.random(20, 50))
                        TestFrame.unitHealthBar:SetHeight(TestFrame:GetHeight() - PowerBar.Height - 1)
                        TestFrame.unitPowerBarBackdrop:SetFrameLevel(5)
                    end

                    if Absorbs.Enabled and TestFrame.unitAbsorbs then
                        if Health.Direction == "RL" then
                            TestFrame.unitAbsorbs:SetReverseFill(true)
                            TestFrame.unitAbsorbs:ClearAllPoints()
                            TestFrame.unitAbsorbs:SetPoint("TOPRIGHT", TestFrame.unitHealthBar:GetStatusBarTexture(), "TOPLEFT")
                            TestFrame.unitAbsorbs:SetPoint("BOTTOMRIGHT", TestFrame.unitHealthBar:GetStatusBarTexture(), "BOTTOMLEFT")
                        elseif Health.Direction == "LR" then
                            TestFrame.unitAbsorbs:SetReverseFill(false)
                            TestFrame.unitAbsorbs:ClearAllPoints()
                            TestFrame.unitAbsorbs:SetPoint("TOPLEFT", TestFrame.unitHealthBar:GetStatusBarTexture(), "TOPRIGHT")
                            TestFrame.unitAbsorbs:SetPoint("BOTTOMLEFT", TestFrame.unitHealthBar:GetStatusBarTexture(), "BOTTOMRIGHT")
                        end
                        TestFrame.unitAbsorbs:SetMinMaxValues(0, 100)
                        TestFrame.unitAbsorbs:SetValue(math.random(20, 50))
                        TestFrame.unitAbsorbs:SetStatusBarColor(unpack(Absorbs.Colour))
                        TestFrame.unitAbsorbs:SetStatusBarTexture(General.ForegroundTexture)
                    end

                    if TestFrame.unitHealthBar then
                        TestFrame.unitHealthBar:SetSize(Frame.Width - 2, Frame.Height - 2)
                        TestFrame.unitHealthBar:ClearAllPoints()
                        TestFrame.unitHealthBar:SetPoint("TOPLEFT", TestFrame, "TOPLEFT", 1, -1)
                        TestFrame.unitHealthBar:SetStatusBarTexture(General.ForegroundTexture)
                        TestFrame.unitHealthBar:SetMinMaxValues(0, 100)
                        TestFrame.unitHealthBar:SetValue(math.random(20, 50))
                        local class = select(2, UnitClass("player"))
                        local classColor = RAID_CLASS_COLORS[class]
                        TestFrame.unitHealthBar:SetStatusBarColor(classColor.r, classColor.g, classColor.b, 1)
                        if Health.Direction == "RL" then
                            TestFrame.unitHealthBar:SetReverseFill(true)
                        elseif Health.Direction == "LR" then
                            TestFrame.unitHealthBar:SetReverseFill(false)
                        end
                    end

                    TestFrame.unitHighLevelFrame = CreateFrame("Frame", nil, TestFrame)
                    TestFrame.unitHighLevelFrame:SetSize(Frame.Width, Frame.Height)
                    TestFrame.unitHighLevelFrame:SetPoint("CENTER", 0, 0)
                    TestFrame.unitHighLevelFrame:SetFrameLevel(999)

                    if TestFrame.unitLeftText then
                        TestFrame.unitLeftText:ClearAllPoints()
                        TestFrame.unitLeftText:SetFont(General.Font, LeftText.FontSize, General.FontFlag)
                        TestFrame.unitLeftText:SetPoint("LEFT", TestFrame.unitHighLevelFrame, "LEFT", LeftText.XOffset, LeftText.YOffset)
                        TestFrame.unitLeftText:SetText(Unit)
                        TestFrame.unitLeftText:Show()
                    end

                    if TestFrame.unitRightText then
                        TestFrame.unitRightText:ClearAllPoints()
                        TestFrame.unitRightText:SetFont(General.Font, RightText.FontSize, General.FontFlag)
                        TestFrame.unitRightText:SetPoint("RIGHT", TestFrame.unitHighLevelFrame, "RIGHT", RightText.XOffset, RightText.YOffset)
                        TestFrame.unitRightText:SetText(UUF:FormatLargeNumber(math.random(10000, 100000)))
                        TestFrame.unitRightText:Show()
                    end

                    if TestFrame.unitTargetMarker and TargetMarker.Enabled then
                        TestFrame.unitTargetMarker:ClearAllPoints()
                        TestFrame.unitTargetMarker:SetSize(TargetMarker.Size, TargetMarker.Size)
                        TestFrame.unitTargetMarker:SetPoint(TargetMarker.AnchorFrom, TestFrame, TargetMarker.AnchorTo, TargetMarker.XOffset, TargetMarker.YOffset)
                        TestFrame.unitTargetMarker:SetTexture("Interface\\TargetingFrame\\UI-RaidTargetingIcon_1")
                        TestFrame.unitTargetMarker:Show()
                    end

                    if Buffs.Enabled and TestFrame.unitBuffs then
                        AddFakeAura(TestFrame.unitBuffs, Buffs.Num or 5, false, Unit)
                    end
                    if Debuffs.Enabled and TestFrame.unitDebuffs then
                        AddFakeAura(TestFrame.unitDebuffs, Debuffs.Num or 3, true, Unit)
                    end
                end
            end
        end
...
function UUF:UpdateTestModeFrames()
    for _, Unit in pairs(Frames) do
        local TestFrame = _G["UUF_" .. Unit]
        if TestFrame then
            local UnitDB = UUF.DB.global[Unit]
            ...
            if UnitDB then
                ...
                TestFrame:SetSize(Frame.Width, Frame.Height)
                TestFrame:SetPoint(Frame.AnchorFrom, Frame.AnchorParent, Frame.AnchorTo, Frame.XPosition, Frame.YPosition)

                if Portrait.Enabled and TestFrame.unitPortrait then
                    TestFrame.unitPortraitBackdrop:ClearAllPoints()
                    TestFrame.unitPortraitBackdrop:SetSize(Portrait.Size, Portrait.Size)
                    TestFrame.unitPortraitBackdrop:SetPoint(Portrait.AnchorFrom, TestFrame, Portrait.AnchorTo, Portrait.XOffset, Portrait.YOffset)
                    TestFrame.unitPortraitBackdrop:SetBackdrop(BackdropTemplate)
                    TestFrame.unitPortraitBackdrop:SetBackdropColor(unpack(General.BackgroundColour))
                    TestFrame.unitPortraitBackdrop:SetBackdropBorderColor(unpack(General.BorderColour))
                    TestFrame.unitPortrait:SetSize(TestFrame.unitPortraitBackdrop:GetHeight() - 2, TestFrame.unitPortraitBackdrop:GetHeight() - 2)
                    TestFrame.unitPortrait:SetPoint("CENTER", TestFrame.unitPortraitBackdrop, "CENTER", 0, 0)
                end
                ...
                if Buffs.Enabled then
                    if not TestFrame.unitBuffs then
                        TestFrame.unitBuffs = CreateFrame("Frame", nil, TestFrame)
                    end
                    TestFrame.unitBuffs:ClearAllPoints()
                    TestFrame.unitBuffs:SetSize(TestFrame:GetWidth(), Buffs.Size)
                    TestFrame.unitBuffs:SetPoint(Buffs.AnchorFrom, TestFrame, Buffs.AnchorTo, Buffs.XOffset, Buffs.YOffset)
                    TestFrame.unitBuffs.size = Buffs.Size
                    TestFrame.unitBuffs.spacing = Buffs.Spacing
                    TestFrame.unitBuffs.num = Buffs.Num
                    TestFrame.unitBuffs.initialAnchor = Buffs.AnchorFrom
                    TestFrame.unitBuffs["growth-x"] = Buffs.GrowthX
                    TestFrame.unitBuffs["growth-y"] = Buffs.GrowthY
                    TestFrame.unitBuffs.filter = "HELPFUL"
                    TestFrame.unitBuffs:Show()

                    for i = 1, #TestFrame.unitBuffs do
                        if i > Buffs.Num then
                            TestFrame.unitBuffs[i]:Hide()
                        end
                    end

                    AddFakeAura(TestFrame.unitBuffs, Buffs.Num, false, Unit)
                else
                    if TestFrame.unitBuffs then
                        for _, button in ipairs(TestFrame.unitBuffs) do
                            button:Hide()
                        end
                        TestFrame.unitBuffs:Hide()
                    end
                end

                -- Update Debuffs
                if Debuffs.Enabled then
                    if not TestFrame.unitDebuffs then
                        TestFrame.unitDebuffs = CreateFrame("Frame", nil, TestFrame)
                    end
                    TestFrame.unitDebuffs:ClearAllPoints()
                    TestFrame.unitDebuffs:SetSize(TestFrame:GetWidth(), Debuffs.Size)
                    TestFrame.unitDebuffs:SetPoint(Debuffs.AnchorFrom, TestFrame, Debuffs.AnchorTo, Debuffs.XOffset, Debuffs.YOffset)
                    TestFrame.unitDebuffs.size = Debuffs.Size
                    TestFrame.unitDebuffs.spacing = Debuffs.Spacing
                    TestFrame.unitDebuffs.num = Debuffs.Num
                    TestFrame.unitDebuffs.initialAnchor = Debuffs.AnchorFrom
                    TestFrame.unitDebuffs["growth-x"] = Debuffs.GrowthX
                    TestFrame.unitDebuffs["growth-y"] = Debuffs.GrowthY
                    TestFrame.unitDebuffs.filter = "HARMFUL"
                    TestFrame.unitDebuffs:Show()
                    for i = 1, #TestFrame.unitDebuffs do
                        if i > Debuffs.Num then
                            TestFrame.unitDebuffs[i]:Hide()
                        end
                    end
                    AddFakeAura(TestFrame.unitDebuffs, Debuffs.Num, true, Unit)
                else
                    if TestFrame.unitDebuffs then
                        for _, button in ipairs(TestFrame.unitDebuffs) do
                            button:Hide()
                        end
                        TestFrame.unitDebuffs:Hide()
                    end
                end
            end
        end
